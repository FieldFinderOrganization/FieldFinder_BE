...
/**
 * Nạp tiền vào ví chủ sân qua PayOS. KHÔNG tin webhook để cộng tiền: khi nhận webhook
 * báo PAID, gọi thẳng PayOS xác nhận trạng thái + số tiền rồi mới cộng ví — chống webhook
 * giả mạo (nạp ví dẫn tới tiền thật rút ra). Cộng đúng-một-lần qua {@code markCreditedIfPending}.
 */
@Service
...
public class WalletTopupService {
    ...
    /**
     * Webhook báo có giao dịch cho {@code transactionId}. Nếu đây là 1 lệnh nạp ví:
     * xác nhận server-side với PayOS, cộng ví đúng-một-lần, trả {@code true} (đã xử lý).
     * Trả {@code false} nếu transactionId KHÔNG phải lệnh nạp ví → caller chạy luồng thường.
     */
    @Transactional
